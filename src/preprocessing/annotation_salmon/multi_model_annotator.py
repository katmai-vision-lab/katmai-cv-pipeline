...
class MultiModelAnnotator:
    ...
    def __init__(
        ...
        """
        Initialize multi-model annotator for salmon detection.
        ...
        # Model weights for salmon detection (2-model system)
        # MegaDetector disabled: trained for terrestrial animals, poor performance on fish
        # Weights adjusted to sum to 1.0 for gdino + detr only
        # Based on bear evaluation, re-normalized without MegaDetector:
        #   gdino: 0.406 / (0.406 + 0.259) = 0.61
        #   detr: 0.259 / (0.406 + 0.259) = 0.39
        self.model_weights = {
            'gdino': 0.61,       # Strong zero-shot performance on "salmon" prompt
            'detr': 0.39,        # Complementary detections
            'megadet': 0.0,      # Not used for salmon
        }

        print("\n" + "="*60)
        print("初始化三文鱼多模型标注系统 (Grounding DINO + DETR)")
        print("="*60)
...
def main():
    ...
    parser.add_argument(
        "--prompt", "-p",
        default="salmon",
        help="Text prompt for detection (default: 'salmon')"
    )
    parser.add_argument(
        "--iou-threshold",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum number of models that must agree (default: 1, since only 2 models active)"
    )
    parser.add_argument(
        "--limit", "-n",
        type=int,
