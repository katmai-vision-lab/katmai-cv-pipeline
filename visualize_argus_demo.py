...
def create_fusion_result(detections_dict, image_size):
    ...
    # 定义模型权重（模拟门控网络的场景自适应权重）
    model_weights = {
        'gdino': 0.45,      # Grounding DINO权重较高
        'owlvit': 0.25,     # OWL-ViT权重较低（用户要求）
        'florence2': 0.30   # Florence-2中等权重
    }
    ...
    for model_name, dets in detections_dict.items():
        ...
        for det in dets:
            all_detections.append({
                'box': det['box'],
                'score': det['score'] * weight,  # 应用模型权重
                'original_score': det['score'],   # 保留原始分数
                'label': det['label'],
                'model': model_name,
                'weight': weight
            })
    ...
    fused = []
    iou_threshold = 0.7  # 提高阈值，保留更多接近的框
...
def main():
    parser = argparse.ArgumentParser(description="ARGUS系统演示")
    parser.add_argument("--image", required=True, help="输入图像路径")
    parser.add_argument("--output", default="demo_results", help="输出目录")
    parser.add_argument("--prompt", default="bear. salmon. fish.", help="检测提示词")
